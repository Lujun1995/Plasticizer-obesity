...
phthte_demo_bmx =
  phthte_demo_bmx %>% 
  select(id = SEQN, year = file_name, gender = RIAGENDR, age = RIDAGEYR, race = RIDRETH3, income = INDFMPIR, bmi_cate = BMDBMIC, bmi = BMXBMI,MEP = URXMEP, MnBP = URXMBP, MiBP = URXMIB, MCPP = URXMC1, MBzP = URXMZP, MEHP = URXMHP, MEHHP = URXMHH, MEOHP = URXMOH) %>%
  mutate(phthte_all = MEP + MnBP + MiBP + MCPP + MBzP + MEHP + MEHHP + MEHHP,
         race = factor(race, levels = c(1, 2, 3, 4, 6, 7), labels = c("mexican_american", "other_hispanic", "non_hispanic_white", "non_hispanic_black", "non_hispanic_asian", "other_race")),
         gender = factor(gender, levels = c(1, 2), labels = c("male", "female")),
         bmi_cate = factor(bmi_cate, levels = c(1, 2, 3, 4), labels = c("underweight", "normal weight", "Overweight","obese")),
...
We selected variables of interest and converted `gender`, `race` and `bmi_cat` into factors. For each individual, the total exposure (`phthte_all`) was calculated as  the sum of exposure for each of the eight phthalates. Then we used `gather` to go from wide format to long format. We took log transformation of `concentrate` and created. 
...
phthalate_bmi %>% filter(phthte == "phthte_all") %>% ggplot(aes(x = log_value, y = overweight_obese)) + geom_point() + geom_smooth()
